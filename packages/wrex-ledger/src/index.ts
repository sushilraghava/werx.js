...
export class LedgerBridge {
  ...
  async buildTx (amount: number, publicKey: string, bip44Index: number, fromAddress: string, toAddress: string) {
    ...
    const lastRef = await wrex.network.loadBalancerApi.getAddressLastAcceptedTransactionRef(fromAddress);

    const { tx, rle } = wrex.keyStore.prepareTx(amount, toAddress, fromAddress, lastRef, 0);
  ...
  async signTransaction(publicKey: string, bip44Index: number, hash: string, ledgerEncodedTx: string) {
    ...
    //const success = wrex.keyStore.verify(publicKey, hash, results.signature);
  ...
  public async getAccountInfoForPublicKeys (ledgerAccounts: { publicKey: string}[]) {
    ...
    if (ledgerAccounts.length > 0) {
      ...
      for (let i = 0; i < ledgerAccounts.length; i++) {
        ...
        wrex.account.loginPublicKey(publicKey);
        const address = wrex.account.address;
        console.log('public', publicKey, address);
        const balance = (await wrex.account.getBalance() || 0);
        const response = {
          address,
          publicKey,
          balance: balance
