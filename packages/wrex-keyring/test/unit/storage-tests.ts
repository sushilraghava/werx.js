...
import {wrexDi} from '@wrex/wrex-core';
...
const testExpect = {
  wrexAddress: 'WREX6C2vbjLkH3wzxVJaEu1fCwLSobRCTQnARZxyo',
...
describe('KeyringController', () => {
  ...
  it('setup-vault-hd-wallet', async () => {
    ...
    expect(testExpect.wrexAddress).to.equal(account1.getAddress());
    expect(testExpect.ethAddress).to.equal(account2.getAddress());

    const encryptedVault = wrexDi.getStateStorageDb().get('vault');
