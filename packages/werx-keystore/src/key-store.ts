...
const WERX_COIN = 2300
const ETH_WALLET_PATH = 60;

const BASE58_ALPHABET = /['123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+/;

//NOTE: During recover account from seed phrase, detect ETH accounts - inform user of derivation path and compatibility?  ETH (reuse ETH accounts) or WERX (ledger support)
const CONSTANTS = {
  BIP_44_WERX_PATH: `m/44'/${WERX_COIN}'/0'/0`,
...
const DERIVATION_PATH_MAP = {
  [DERIVATION_PATH.WERX]: CONSTANTS.BIP_44_WERX_PATH,
...
export const PERSONAL_SIGN_PREFIX = `\u0019WERX Signed Message:\n`;
...
export class KeyStore {
  ...
  getPrivateKeyFromMnemonic (mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.WERX) {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  getMasterKeyFromMnemonic (mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.WERX): HDKey {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  getWerxAddressFromPrivateKey (privateKeyHex: string) {
    return this.getWerxAddressFromPublicKey(this.getPublicKeyFromPrivate(privateKeyHex));
  }
  ...
  getWerxAddressFromPublicKey (publicKeyHex: string) {
