import {crossPlatformDi} from '@wrex/wrex-core';
import {loadBalancerApi, Transaction, PendingTx, CbTransaction, TransactionV2} from '@wrex/wrex-network';
import {WrexAccount} from './wrex-account';
...
export class WrexMonitor {

  private memPoolChange$ = new Subject<WrexWalletMonitorUpdate>();
  private lastTimer: number;
  private pendingTimer = 0;
  private waitForMap: { [hash:string]: WaitFor } = {};

  constructor (private wrexAccount: WrexAccount) {
  ...
  async addToMemPoolMonitor (value: PendingTx | string): Promise<Transaction | TransactionV2> {
    const networkInfo = this.wrexAccount.networkInstance.getNetwork();
  ...
  async getLatestTransactions (address: string, limit?: number, searchAfter?: string): Promise<(Transaction | TransactionV2)[]> {
    const cTxs = await this.wrexAccount.networkInstance.getTransactionsByAddress(address, limit, searchAfter);    
  ...
  async getMemPoolFromMonitor(address?: string): Promise<PendingTx[]> {
    address = address || this.wrexAccount.address;
    const networkInfo = this.wrexAccount.networkInstance.getNetwork();
  ...
  private transformPendingToTransaction (pending: PendingTx): Transaction | TransactionV2 {
    ...
    const networkVersion = this.wrexAccount.networkInstance.getNetworkVersion();
  ...
  private async processPendingTxs () {
    ...
    for (let index = 0; index < pool.length; index++) {
      ...
      if (cbTx) {
        ...
        try {
          beTx = await this.wrexAccount.networkInstance.getTransaction(txHash);
        } catch(e) {}
