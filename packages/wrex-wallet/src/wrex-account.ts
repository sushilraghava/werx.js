import {keyStore, KeyTrio, PostTransaction, PostTransactionV2} from '@wrex/wrex-keystore';
import {WREX_DECIMALS} from '@wrex/wrex-core';
import {globalWrexNetwork, GlobalWrexNetwork, WrexNetwork, NetworkInfo, PendingTx, TransactionReference, Transaction, TransactionV2} from '@wrex/wrex-network';
import {BigNumber} from 'bignumber.js';
import {Subject} from 'rxjs';
import {networkConfig} from './network-config';

export class WrexAccount {

  private m_keyTrio: KeyTrio;
  private sessionChange$ = new Subject<boolean>();
  private network: WrexNetwork | GlobalWrexNetwork;

  constructor(network: WrexNetwork | GlobalWrexNetwork) {
    this.network = network || globalWrexNetwork;
  }
  ...
  get address () {
    ...
    if (!address) {
      throw new Error('Need to login before calling methods on wrex.account');
    }
  ...
  loginPrivateKey (privateKey: string) {
    ...
    const address = keyStore.getWrexAddressFromPublicKey(publicKey);
  ...
  validateWrexAddress (address: string) {
    return keyStore.validateWrexAddress(address)
  }
  ...
  async getBalanceFor (address: string) {
    ...
    if (addressObj && !isNaN(addressObj.balance)) {
      return new BigNumber(addressObj.balance).multipliedBy(WREX_DECIMALS).toNumber();
    }
  ...
  async transferWrex (toAddress: string, amount: number, fee = 0, autoEstimateFee = false): Promise<PendingTx> {
    let normalizedAmount = Math.floor(new BigNumber(amount).multipliedBy(WREX_DECIMALS).toNumber());
  ...
  async generateBatchTransactions(transfers: TransferBatchItem[], lastRef?: TransactionReference) {
    if (this.network.getNetworkVersion() === '1.0') {
      throw new Error('transferWrexBatch not available for mainnet 1.0');
    }
  ...
  async transferWrexBatch(transfers: TransferBatchItem[], lastRef?: TransactionReference) {
