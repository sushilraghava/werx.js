...
const WREX_COIN = 2300
const ETH_WALLET_PATH = 60;

const BASE58_ALPHABET = /['123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+/;

//NOTE: During recover account from seed phrase, detect ETH accounts - inform user of derivation path and compatibility?  ETH (reuse ETH accounts) or WREX (ledger support)
const CONSTANTS = {
  BIP_44_WREX_PATH: `m/44'/${WREX_COIN}'/0'/0`,
...
const DERIVATION_PATH_MAP = {
  [DERIVATION_PATH.WREX]: CONSTANTS.BIP_44_WREX_PATH,
...
export const PERSONAL_SIGN_PREFIX = `\u0019WREX Signed Message:\n`;
...
export class KeyStore {
  ...
  getPrivateKeyFromMnemonic (mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.WREX) {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  getMasterKeyFromMnemonic (mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.WREX): HDKey {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  validateWrexAddress (address: string) {
    ...
    const validPrefix = address.substr(0, 3) === 'WREX';
  ...
  getWrexAddressFromPrivateKey (privateKeyHex: string) {
    return this.getWrexAddressFromPublicKey(this.getPublicKeyFromPrivate(privateKeyHex));
  }
  ...
  getWrexAddressFromPublicKey (publicKeyHex: string) {
