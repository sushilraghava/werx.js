...
describe('Key Store', () => {
  ...
  it('IsValid WREX address', async () => {
    ...
    const result = wrex.keyStore.validateWrexAddress(testData.WREX_ADDRESS);
  ...
  it('Public key from Private', () => {
    ...
    const result = wrex.keyStore.getPublicKeyFromPrivate(testData.PRIVATE_KEY);
  ...
  it('Compact Public key from Private', () => {
    ...
    const result = wrex.keyStore.getPublicKeyFromPrivate(testData.PRIVATE_KEY, true);
  ...
  it('WREX address from Public',  () => {
    ...
    const result = wrex.keyStore.getWrexAddressFromPublicKey(testData.PUBLIC_KEY);
    chai.expect(result).to.equal(testData.WREX_ADDRESS);
  });

  it('Private Key from Mnemonic Seed Phrase',  () => {

    const result = wrex.keyStore.getPrivateKeyFromMnemonic(testData.SEED_PHRASE);
    chai.expect(result).to.equal(testData.SEED_PRIVATE_KEY);

    // console.log(wrex.keyStore.generateSeedPhrase());
  });
...
function loadJSON () {
  return {
    ...
    "WREX_ADDRESS": "WREX5WtmeekZLUS4vxCDhe9safyE6wFQ94EaczotN",
