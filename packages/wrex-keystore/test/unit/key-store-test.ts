...
describe('Key Store', () => {
  ...
  it('IsValid WREX address', async () => {

    const result = keyStore.validateWrexAddress(testData.WREX_ADDRESS);

    expect(result).to.equal(true);
  });

  it('IsInvalid WREX address', async () => {

    //Empty
    const result = keyStore.validateWrexAddress('');
    expect(result).to.equal(false);

    //Bad length
    const result0 = keyStore.validateWrexAddress('WREX2itmeekZLUS4vxCDhe9safyE6wFQ94EaczotNn');
    expect(result0).to.equal(false);

    //Bad prefix
    const result1 = keyStore.validateWrexAddress('DOG2itmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
    expect(result1).to.equal(false);

    //Bad Parity
    const result2 = keyStore.validateWrexAddress('WREXJitmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
    expect(result2).to.equal(false);

    //Bad Base58 Match
    const result3 = keyStore.validateWrexAddress('WREX20itmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
  ...
  it('WREX address from Public',  () => {

    const result = keyStore.getWrexAddressFromPublicKey(testData.PUBLIC_KEY);
    expect(result).to.equal(testData.WREX_ADDRESS);
  });
