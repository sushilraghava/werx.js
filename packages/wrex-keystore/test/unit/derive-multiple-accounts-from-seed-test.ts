...
const testExpect = {
  ...
  wrexAddress: 'WREX27RfNfVXEtsLBt6aZf6BYQktyeB9NUX3bw7ma',
...
describe('derive multiple accounts from seed', () => {
  ...
  it('Mnemonic seed phrase', async () => {
    ...
    for (let i=0; i < 10; i++) {
      ...
      const wrexAddress = keyStore.getWrexAddressFromPrivateKey(key);

      const wrexAddress2 = keyStore.getWrexAddressFromPublicKey(pKey2);

      const ethAddress = keyStore.getEthAddressFromPrivateKey(key);

      console.log(i, ':', key, wrexAddress, wrexAddress2, ethAddress);
      console.log('   ', pKey, pKey2);

      results.push({key, wrexAddress, ethAddress})
    }

    const result = results[testExpect.index];

    expect(testExpect.key).to.equal(result.key);
    expect(testExpect.wrexAddress).to.equal(result.wrexAddress);
