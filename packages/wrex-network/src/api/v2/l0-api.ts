import {RestApi} from '@wrex/wrex-core';
...
export class L0Api {
  ...
  async getTotalSupply () {
    return this.service.$get<TotalSupplyV2>('/wrex/total-supply');
  }

  async getTotalSupplyAtOrdinal (ordinal: number) {
    return this.service.$get<TotalSupplyV2>(`/wrex/${ordinal}/total-supply`);
  }
 
  async getAddressBalance (address: string) {
    return this.service.$get<L0AddressBalance>(`/wrex/${address}/balance`);
  }

  async getAddressBalanceAtOrdinal (ordinal: number, address: string) {
    return this.service.$get<L0AddressBalance>(`/wrex/${ordinal}/${address}/balance`);
  }
